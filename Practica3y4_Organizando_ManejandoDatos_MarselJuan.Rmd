...
```{r}
pew_religion_tidy <- pew_religion %>%
  gather(key = "income", value ="freq", factor_key = T, -religion)

print(pew_religion_tidy)
```
...
ej2_tidy <- raw %>%
  ...
  mutate(week = as.integer(week), date = as.Date(date.entered) + weeks(week - 1))
...
```{r}
ej3_tidy <- raw %>%
  gather(key = "col", value = "casos", m014:fu) %>%
  separate(col = "col", sep = 1, into = c("sexo", "edad"))
...
```{r}
ej4_tidy <- raw %>%
  pivot_longer(names_to = "dia", values_to = "temperatura" , cols = d1:d31, names_prefix = "d") %>%
  drop_na(temperatura) %>%
  pivot_wider(names_from = element, values_from = temperatura) %>%
  unite(col = "fecha", year, month, dia, sep = "-") %>%
